...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        print(f"Type of im0s: {type(im0s)}")  # im0s의 타입을 로그로 출력
        if isinstance(im0s, list):
            print(f"Number of images in im0s: {len(im0s)}")  # im0s 리스트의 길이를 로그로 출력
        else:
            print("im0s is not a list")  # im0s가 리스트가 아니라면 로그로 출력
        with dt[0]:
        ...
        if not isinstance(im0s, list):
            im0s = [im0s]  # Batch mode 활성화

        print(f"Type of im0s: {type(im0s)}")
        if isinstance(im0s, np.ndarray):
            im0 = im0s
        else:
            print("Error: im0s is not a valid numpy array.")
            continue
        ...
        for i, det in enumerate(pred):  # 이미지별
            ...
            if isinstance(im0s, list):
                im0 = im0s[i] if i < len(im0s) else im0s[-1]  # 리스트에서 적절한 이미지 선택
                print(f"Selected image index: {i}")  # 선택된 이미지 인덱스를 로그로 출력
            else:
                im0 = im0s  # 단일 이미지 처리

            print(f"Type of im0: {type(im0)}")  # im0의 타입을 로그로 출력
            if not isinstance(im0, np.ndarray):
                print("Error: im0 is not a valid numpy array.")
                continue  # im0이 NumPy 배열이 아닌 경우 스킵

            # Annotator를 초기화하기 전에 im0가 numpy 배열인지 다시 한 번 확인
            if isinstance(im0, np.ndarray):
                annotator = Annotator(im0, line_width=line_thickness)  # 에러가 발생한 부분
            else:
                print("Annotator input is not a numpy array.")
                continue
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in det:
                    if names[int(cls)] == 'drone':  # 드론 감지 시 특별 처리
            ...
            if save_img:
                ...
                if dataset.mode == 'image':
                    ...
                    # 비디오 저장을 종료하고 전송하기 위한 로직 (예: 데이터셋 처리의 마지막에 위치)
                    if vid_writer[i] is not None:
                        vid_writer[i].release()  # 비디오 작성자 해제
                        vid_writer[i] = None
        ...
        if dataset.mode == 'video':
            if vid_writer[i] is not None:
                vid_writer[i].release()  # 비디오 작성자 해제
                vid_writer[i] = None
                print(f"Video file saved at {save_path}")
                send_detection(save_path, LAST_DETECTED_TIME)
